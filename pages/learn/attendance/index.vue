    ...
    <div class="relative z-10 container mx-auto px-4 py-8">
      <MotionCard class="mb-8 bg-gradient-to-r from-gray-800/50 to-gray-700/50 backdrop-blur-xl border border-gray-600/30" :initial="{ opacity: 0, y: -20 }" :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 600 }">
        <div class="p-8 flex items-center justify-between">
          <div>
            <h1 class="text-4xl md:text-5xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-violet-400 bg-clip-text text-transparent mb-2">My Attendance</h1>
            <p class="text-gray-300 text-lg">View your attendance records across all courses</p>
          </div>
        </div>
      </MotionCard>
      <MotionCard class="bg-gradient-to-br from-gray-800/50 to-gray-700/50 backdrop-blur-xl border border-gray-600/30" :initial="{ opacity: 0, y: 50 }" :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 600, delay: 200 }">
        <div class="p-8">
          <div v-if="attendanceRecords.length" class="space-y-6">
            <div v-for="record in attendanceRecords" :key="record.id" class="bg-gray-700/40 rounded-xl px-6 py-4">
              <div class="flex items-center justify-between">
                <div>
                  <h3 class="text-lg font-semibold text-white">{{ getCourseName(record.courseId) }}</h3>
                  <p class="text-gray-400">{{ formatDate(record.date) }}</p>
                </div>
          ...
          <div v-else class="text-center py-12">
            ...
            <p class="text-gray-400">Your attendance records will appear here once teachers start taking attendance.</p>
          </div>
        </div>
      </MotionCard>
    </div>
...
onMounted(() => { attendanceStore.fetchAttendanceRecords(); courseStore.fetchCourses(); });
