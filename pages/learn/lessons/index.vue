    ...
    <div class="relative z-10 container mx-auto px-4 py-8">
      <MotionCard class="mb-8 bg-gradient-to-r from-gray-800/50 to-gray-700/50 backdrop-blur-xl border border-gray-600/30" :initial="{ opacity: 0, y: -20 }" :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 600 }">
        <div class="p-8 flex items-center justify-between">
          <div>
            <h1 class="text-4xl md:text-5xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-violet-400 bg-clip-text text-transparent mb-2">My Lessons</h1>
            <p class="text-gray-300 text-lg">View and complete lessons for your courses</p>
          </div>
        </div>
      </MotionCard>
      <MotionCard class="bg-gradient-to-br from-gray-800/50 to-gray-700/50 backdrop-blur-xl border border-gray-600/30" :initial="{ opacity: 0, y: 50 }" :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 600, delay: 200 }">
        <div class="p-8">
          <div v-if="lessons.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <MotionTile v-for="lesson in lessons" :key="lesson.id" class="bg-gradient-to-br from-gray-700/50 to-gray-600/50 backdrop-blur-xl border border-gray-500/30 p-6 rounded-xl hover:border-purple-500/50 transition-all duration-300 cursor-pointer" :hover="{ scale: 1.02, y: -5 }" @click="openLesson(lesson)">
          ...
          <div v-else class="text-center py-12">
            ...
            <p class="text-gray-400 mb-6">Lessons will appear here as your teacher adds them</p>
          </div>
        </div>
      </MotionCard>
    </div>
...
const openLesson = (lesson: any) => {
  // Use router to navigate to lesson detail
  window.location.href = `/learn/lessons/${lesson.id}`;
};
