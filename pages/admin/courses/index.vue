    ...
    <div class="relative z-10 container mx-auto px-4 py-8">
      <MotionCard class="mb-8 bg-gradient-to-r from-gray-800/50 to-gray-700/50 backdrop-blur-xl border border-gray-600/30" :initial="{ opacity: 0, y: -20 }" :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 600 }">
        <div class="p-8 flex items-center justify-between">
          <div>
            <h1 class="text-4xl md:text-5xl font-bold bg-gradient-to-r from-purple-400 via-pink-400 to-violet-400 bg-clip-text text-transparent mb-2">All Courses</h1>
            <p class="text-gray-300 text-lg">Manage all courses in the system</p>
          </div>
        </div>
      </MotionCard>
      <MotionCard class="bg-gradient-to-br from-gray-800/50 to-gray-700/50 backdrop-blur-xl border border-gray-600/30" :initial="{ opacity: 0, y: 50 }" :enter="{ opacity: 1, y: 0 }" :transition="{ duration: 600, delay: 200 }">
        <div class="p-8">
          <div v-if="courses.length" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <MotionTile v-for="course in courses" :key="course.id" class="bg-gradient-to-br from-gray-700/50 to-gray-600/50 backdrop-blur-xl border border-gray-500/30 p-6 rounded-xl hover:border-purple-500/50 transition-all duration-300" :hover="{ scale: 1.02, y: -5 }">
              ...
              <p class="text-gray-300 text-sm mb-4 line-clamp-3">{{ course.description }}</p>
              ...
              <div class="flex items-center justify-between text-xs text-gray-400">
                ...
                <MotionButton :to="`/admin/courses/${course.id}`" class="text-purple-400 hover:text-purple-300" :hover="{ scale: 1.1 }">View</MotionButton>
              </div>
            </MotionTile>
          </div>
          <div v-else class="text-center py-12">
            ...
            <p class="text-gray-400">No courses have been created in the system yet.</p>
          </div>
        </div>
      </MotionCard>
    </div>
...
const courses = ref<any[]>([]); // TODO: Fetch real courses
const getStatusClass = (status: string) => {
  switch (status) {
    case 'active': return 'bg-green-500/20 text-green-300 border border-green-500/30';
    case 'inactive': return 'bg-red-500/20 text-red-300 border border-red-500/30';
    case 'draft': return 'bg-yellow-500/20 text-yellow-300 border border-yellow-500/30';
